...
popularity_duration <- popularity_duration %>% filter(Popularity>0)
ggplot(popularity_duration, aes(x=DurationMin, y=Popularity, color=DurationCategory)) +
  geom_point() +
  geom_smooth(method="lm", se=FALSE, formula=y ~ x, aes(group=DurationCategory), color="black") +
  labs(title="Regresijski grafikon - Popularity vs DurationMin", x="DurationMin", y="Popularity") +
  theme_minimal() +
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14),
        plot.title=element_text(size=16, hjust=0.5))
ggsave("lin_regresija.png")
```
![](graf1.png)
<br>
<br>
...
Iz grafa "Regresijski Grafikon - Popularity vs DurationMin" koji prikazuje regresiju Popularitiy-a u odnosu na Duration, lako se zaključi da je Duration dobra regresijska varijabla za Popularity. U kategoriji kratkih pjesama vidimo rast Popularity-a sa rastom trajanja, dok u ostale dvije skupine, srednje-duge pjesme i duge pjesme, vidimo jednaki takav pad varijable Popularity u odnosu na regresor DurationMin
<br>
<br>
...
```{r, include=FALSE}
selected_years <- data[data$ReleaseYear %in% c(2017, 2018, 2019), ]
ggplot(selected_years, aes(x = DurationMin, fill = as.factor(ReleaseYear))) +
  geom_density(alpha = 0.5) +
  labs(title = "Odnos Vremena Trajanja Pjesama za Godine 2017, 2018, i 2019",
       x = "Trajanje (u minutama)",
       y = "Gustoća") +
  scale_x_continuous(breaks = seq(0, 10, by = 1)) +  # Postavljanje oznaka na osi x svakih 1 minutu
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 12, face = "bold"),
    axis.title.x = element_text(size = 12, face = "bold"),
    ...
    axis.ticks = element_line(size = 0.5),  # Podešavanje debljine oznaka na osi x
    panel.grid.major = element_line(color = "lightgray", linetype = "dashed", size = 0.5),
    panel.grid.minor = element_blank()
  )
ggsave("duration_years.png")
```
![](duration_years.png)
<br>
<br>
...
Iz analize grafa odnosa vremena trajanja pjesama za godine 2017., 2018. i 2019., uočavamo da je za sve tri godine najveći broj pjesama trajao između 3 i 4 minute. Ovaj trend posebno je izražen u 2017. godini, gdje gustoća pjesama u tom vremenskom intervalu dominira u usporedbi s preostalim godinama. Primjećujemo i značajan porast broja kraćih pjesama, od 2 do 3 minute, tijekom 2019. godine, čime se nastavlja trend započet u 2018. godini. Takve kraće pjesme karakterizirane su lakoćom pamćenja i postaju sve popularnije s godinama.
U skladu s navedenim, zaključujemo da je s vremenom postao popularniji i učestaliji tip pjesama koji traje kraće, trenutno najpopularniji oko 3 minuta, no s tendencijom pada vremena izvođenja.

<br>
<br>

```{r, include=FALSE}
ggplot(data, aes(x = Genre, y = DurationMin, fill = Genre)) +
  geom_violin() +
  labs(title = "Distribucija trajanja pjesama po žanrovima", x = "Žanr", y = "Trajanje (min)") +
  theme_minimal()
...
Analiza distribucije trajanja pjesama unutar različitih glazbenih žanrova, prema našem datasetu, otkriva značajne razlike između žanrova, osobito između rapa i popa. Možemo primjetiti je da se rap ističe svojom širokom distribucijom trajanja pjesama, pokrivajući širok spektar vremenskih intervala. Ovo ukazuje na raznolikost u strukturi i formi rap pjesama, sugerirajući fleksibilnost žanra u pogledu trajanja.
Nasuprot tome, žanr pop pokazuje tendenciju koncentracije oko prosječnog trajanja od približno tri minute. Ovo bi moglo odražavati standardiziranu formu popularnih pjesama, koje su često usmjerene na radio emitiranje i komercijalnu dostupnost. Zanimljivo je primijetiti da se žanr 'latin' uvelike podudara s popom po distribuciji trajanja, što ukazuje na sličnosti u strukturalnim konvencijama između ova dva žanra.
Dodatno, naša analiza ukazuje da žanr R&B, u prosjeku, ima najduže trajanje pjesama unutar analiziranih žanrova. Ovo može odražavati jedinstvene umjetničke i kreativne izbore koji karakteriziraju R&B glazbu, kao što su produženi instrumentalni dijelovi ili složenija lirska struktura.
Ovi uvidi pružaju zanimljiv pogled na različite glazbene prakse i konvencije unutar svakog žanra, ističući kako se umjetničke preferencije i industrijski standardi mogu odražavati kroz osnovne elemente poput trajanja pjesme.
...
ggplot(data, aes(x = Genre, y = DanceAbility, fill = Genre)) +
  ...
  labs(title = "Odnos Danceabilitya po žanrovima", x = "Žanr", y = "Danceability")
ggsave("dance.png")
```
![](dance.png)

<br>
<br>Graf "Odnos Danceabilitya po Žanrovima" pruža vizualni uvid u raspodjelu danceabilitya među različitim glazbenim žanrovima. Svaki žanr predstavljen je na x-osi, dok je danceability prikazan na y-osi, čime se omogućava usporedba plesnosti među žanrovima.

Gledajući graf možemo vidno primjetiti kako rock žanr ima daleko najniži prosječni danceability, što odražava njegove karakteristične glazbene elemente manje prikladne za ples. S druge strane, rap i latin se izdvajaju kao žanrovi s najvišim prosječnim danceabilityem, sugerirajući da glazba u ovim žanrovima često potiče na ples i dinamičnost.
**Analiza grafa**

Graf "Odnos Danceabilitya po Žanrovima" pruža vizualni uvid u raspodjelu danceabilitya među različitim glazbenim žanrovima. Svaki žanr predstavljen je na x-osi, dok je danceability prikazan na y-osi, čime se omogućava usporedba plesnosti među žanrovima.
Gledajući graf možemo vidno primjetiti kako rock žanr ima daleko najniži prosječni danceability, što odražava njegove karakteristične glazbene elemente manje prikladne za ples. S druge strane, rap i latin se izdvajaju kao žanrovi s najvišim prosječnim danceabilityem, sugerirajući da glazba u ovim žanrovima često potiče na ples i dinamičnost.
